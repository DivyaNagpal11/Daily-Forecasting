...
def subset_data(full_df):
    ...
    for i in all_be_list:
        ...
        mask = modified_df['posted_date'] >= start_date
...
#    last_day_data=pd.DataFrame(last_day_data)
...
#    subset_df.reset_index()
    ...
    if(len(outlier_indices_list)>0):
        ...
        #new_df_monthly.loc[outlier_indices_list, 'total_charge'] = median
...
def all_model_combined(new_df_monthly,best_model_name,be_name):
    ...
    outlier_data,test = new_df_monthly[:-92], new_df_monthly[-92:]
#    count_data_points=len(new_df_monthly)
#    n_steps_in, n_steps_out =int(count_data_points-148),92
#    
#    c=0
#    i=0
#    while True:
...
#        c=c+1
#
#        print(i, end_ix,out_end_ix,len(new_df_monthly[-end_ix:]),len(new_df_monthly[-out_end_ix:-end_ix]))
#        
#        outlier_data,test = new_df_monthly[i:end_ix], new_df_monthly[end_ix:out_end_ix]
    ...
    if best_model_name =="SARIMAX":
        train_mape_sarimax,val_mape_sarimax,test_dates,fitted_mean_sarimax,fitted_mean_down,fitted_mean_up=DSX.sarimax_model_daily(train,val,test,be_name)
    ...
    if best_model_name =="PROPHET":    
        train_mape_prophet,val_mape_prophet,test_dates,fitted_mean_prophet,fitted_mean_down,fitted_mean_up=DPH.daily_prophet(train[["total_charge","is_extended","Is_Month_End","LastButOneDay","LastSecDay",'13th_Day','31st_Day']],val[["total_charge","is_extended","Is_Month_End","LastButOneDay","LastSecDay",'13th_Day','31st_Day']],test[["total_charge","is_extended","Is_Month_End","LastButOneDay","LastSecDay",'13th_Day','31st_Day']],be_name)
...
#    
#def report_file(mape_res,new_df_monthly):
#    res=pd.DataFrame()
#    test_predicted_values=pd.DataFrame()
#    for k in range(0,mape_res.set_no.max()):
#        start_date=mape_res["test_dates"][k][0]
#        end_date=mape_res["test_dates"][k][-1]
#        mask = (new_df_monthly.index >= start_date) & (new_df_monthly.index <= end_date)
#        subset_df= ['{:0,.0f}'.format(i) for i in new_df_monthly['total_charge'].loc[mask]]
#        a=mape_res["test_dates"][k][:].strftime("%d-%b-%Y")
#        b=['{:0,.0f}'.format(i) for i in mape_res['test_predicted'][k][:]]
#        test_predicted_values=test_predicted_values.append(b)
#        lens=len(b)
#        res = res.append(pd.DataFrame({'billing_entity': np.repeat(mape_res['billing_entity'][k], lens),
#                            'set_no': np.repeat(mape_res['set_no'][k], lens),
#                            'model': np.repeat(mape_res['model'][k], lens),
#                            'predicted_dates':np.repeat(a[0],lens),
#                            'test_dates': chainer(a),
#                            'actual_charges':subset_df}))  #'test_predicted': chainer(b1)
#    res["test_predicted"]=test_predicted_values
#                            
#    return res


def report_generation1(report_data,best_model_res,timestampStr,be_name):
    x1=report_data[:-92]#.reset_index()
    x1["prediction"]='FALSE'
...
########################## Main Function ####################################
def main(data_path):
    data_charges = get_data(data_path)
...
#    full_df = create_overall_be(new_df,overall_be, not_include_be_list)
    new_df = create_granular_col(new_df)
    ...
    #Condition 1:
    new_full_df.drop(list(df_date[df_date.days_count < 230]['billing_entity']), axis=0, inplace=True)
    current_data_summary.drop(list(df_date[df_date.days_count < 230]['billing_entity']), axis=0, inplace=True)
    current_data_summary=current_data_summary.reset_index()
    
    #Condition 2:
    new_full_df.drop(list(current_data_summary[current_data_summary.days_missing > 0]['billing_entity']), axis=0, inplace=True)
    drop_list=list(current_data_summary[current_data_summary.days_missing > 0]['billing_entity'])
...
#    a=['Adventist Medical Center - Hanford and Selma','Glendale Adventist Medical Center',
#    a=['Sonora Regional Medical Center']
#    a=[sys.argv[1]]
  
    csv_folder=r"feb/"
    if not os.path.exists(csv_folder):
        os.makedirs(csv_folder)
    best_model_file=pd.read_csv("daily_best_model_feb_with_reg.csv")
    final_report1=pd.DataFrame()
#    report_file_generated=pd.DataFrame()
#    new_full_df.billing_entity.unique()
    for k in best_model_file.index[0:2]:
        be_name=best_model_file["be_name"].loc[k]
        best_model_name=best_model_file["best_model"].loc[k]
        print(k,be_name,best_model_name)
        
        
        new_df_monthly=new_full_df[new_full_df.billing_entity == be_name]
        ...
        last_month = new_df_monthly["posted_date"].loc[new_df_monthly.index[-1]].month
        last_year = new_df_monthly["posted_date"].loc[new_df_monthly.index[-1]].year
        last_day = new_df_monthly["posted_date"].loc[new_df_monthly.index[-1]].day
#        last_day=calendar.monthrange(last_year,last_month)[1]
        date1=datetime(last_year,last_month,last_day)
        date2=date1+relativedelta(days=92)
        dates_list=[]
        for dt in daterange(date1, date2):
            dates_list.append(dt.strftime("%Y-%m-%d"))
        new_df_monthly=new_df_monthly.append(pd.DataFrame(dates_list,columns=["posted_date"]),ignore_index=True)
        new_df_monthly['posted_date']=pd.to_datetime(new_df_monthly['posted_date'])
#        new_df_monthly=create_granular_col(new_df_monthly)
...
#        new_df_monthly['27th_Day']= [1 if x==27 else 0 for x in new_df_monthly.index.day]
        ...
        best_model_res,report_data=all_model_combined(new_df_monthly,best_model_name,be_name)
        ...
        report1=report_generation1(report_data,best_model_res,timestampStr,be_name)
...
#    walk_forward_file.to_csv("daily_walk_forward_for_all_be"+".csv")
