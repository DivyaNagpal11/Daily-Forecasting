...
def daily_prophet(train_set,val_set,test_set,be_name,set_no):
...
#            future1['27th_Day'] =pd.Series(x['27th_Day'].values)
    ...
    graph_path_tso = r"daily_prophet_walk_forward_val_set_46_new_regressors_test/"+be_name+"/"
    ...
    layout = go.Layout(go.Layout(title='Billing Entity : {} <br>Train MAPE: {} Val MAPE: {} Test MAPE: {} <br>PROPHET'.format(be_name,round(result_table.train_mape[0],2),round(result_table.val_mape[0],2),test_results),yaxis=dict(title="Daily Charges", zeroline=False),
